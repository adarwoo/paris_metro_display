 ...
 */
static char _log_buffer[_LOG_SIZED_FOR(_LOG_MAX_TRACE)];

/** Place holder for the process name */
static char _log_process_name[_LOG_SIZED_FOR(_LOG_MAX_PROCESS_NAME_LENGTH)];
 ...
 *****************************************************************************
 */
const char * const log_get_truncated_string(
   const char *p,
   int maxLength,
   const char * * pTruncationMarkBuffer )
{
   static const char wasTruncated[]="...";

   size_t l = strlen(p) - maxLength - 1 /* \0 */;

   if ( l > 0 )
   {
      *pTruncationMarkBuffer = wasTruncated;
      return &p[l+sizeof(wasTruncated)];
   }
 ...
 */
bool _log_filter_trace( const char *domain )
{
   // Empty domain means display in all cases.
   if ( ! domain )
   {
      return _log_level >= _log_type;
   }
...
void _log_trace(const char* domain, const char *format, ...)
{
   ...
   if ( _log_full || _log_last.thread_id != current_thread )
   {
      ...
      snprintf( threadIdBuffer, sizeof(threadIdBuffer), " 0x%lx ", current_thread );
   ...
   snprintf( lineNumberBuffer, sizeof(lineNumberBuffer), "%ld} ", _log_line );
